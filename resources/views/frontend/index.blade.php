@extends('frontend.layout.layout')
@section('content')
    <!-- ======= Hero Section ======= -->
    <section>
        <div class="hero-section">
            <div class="row hero">
                <div class="col-sm-12 col-md-7 hero-content-container">
                    <div class="hero-content">
                        <h1>Simple. Find your car<br>
                            parts in one click…</h1>
                        <h3>Request your parts online and we will send
                            you a selection to choose from.</h3>
                    </div>
                </div>
                @if (session()->has('success'))
                    <div class="alert alert-success">
                        @if(is_array(session('success')))
                            <ul>
                                @foreach (session('success') as $message)
                                    <li>{{ $message }}</li>
                                @endforeach
                            </ul>
                        @else
                            {{ session('success') }}
                        @endif
                    </div>
                @endif
                @if ($errors->any())
                    <div class="alert alert-danger">
                        <ul>
                            @foreach ($errors->all() as $error)
                                <li>{{ $error }}</li>
                            @endforeach
                        </ul>
                    </div>
                @endif
                <div class="col-sm-12 col-md-5 enter-reg">
                    <div class="reg-container">
                        <h1>ENTER<br>YOUR <br>REG</h1>
                        <form method="post" action="{{route('find.model')}}" name="registrationform">
                            @csrf
                            <div class="box">
                                <img src="{{asset('public/frontend/assets/img/european-union.svg')}}" alt="">
                                <input type="text" name="registrationNumber" id="registrationNumber">
                                <span></span>

                            </div>

                            <input type="submit" class="findData"></input>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- ======= End Hero ======= -->

    <!-- ======Our features  ======= -->
    <div class="container">
        <div class="row">
            <div class="col-sm-6 col-md-3">
                <div class="feature-card">
                    <img src="{{asset('public/frontend/assets/img/thumbs-up-hand-symbol.png')}}" alt=""><br><br>
                    <h4>Guaranteed parts<br>with warranty</h4>
                </div>
            </div>
            <div class="col-sm-6 col-md-3">
                <div class="feature-card">
                    <img src="{{asset('public/frontend/assets/img/fast-delivery.png')}}" alt=""><br><br>
                    <h4>Quick <br>delivery</h4>
                </div>
            </div>
            <div class="col-sm-6 col-md-3">
                <div class="feature-card">
                    <img src="{{asset('public/frontend/assets/img/shield.png')}}" alt=""><br><br>
                    <h4>Certified <br>suppliers</h4>
                </div>
            </div>
            <div class="col-sm-6 col-md-3">
                <div class="feature-card">
                    <img src="{{asset('public/frontend/assets/img/25.png')}}" alt=""><br><br>
                    <h4>25 years <br>experience</h4>
                </div>
            </div>
        </div>
    </div>
    <!-- ====== End Our features  ======= -->

    <!-- ====== How it Works section  ======= -->
    <div class="container how-it-works">
        <div class="steps-heading">
            <hr></hr>
            <h2>How It Works</h2>
        </div>

        <div class="row steps1">
            <div class="col-md-6 step1">
                <div class="step-text">
                    <h5>Enter<br>your<br>reg </h5>
                </div>
            </div>
            <div class="col-md-6 step2">
                <div class="step-text">
                    <h5>Choose<br>your<br>part</h5>
                </div>
            </div>
        </div>
        <div class="row steps">
            <div class="col-md-4 step3">
                <div class="step-text">
                    <h5>Receive<br>images<br>and<br>pricing</h5>
                </div>
            </div>
            <div class="col-md-4 step4">
                <div class="step-text">
                    <h5>Get<br>a<br>quote</h5>
                </div>
            </div>
            <div class="col-md-4 step5">
                <div class="step-text">
                    <h5>Pay<br>online</h5>
                </div>
            </div>
        </div>
    </div>
    <!-- ======End  How it Works section  ======= -->


    <!-- ====== Mobile How it Works section  ======= -->
    <div class="how-it-mobile">
        <h3 class="text-center font-weight-bold">Top Vehicle Brands</h3>
        <section class="how-it-cards slider1 container">
            <div class="slide1"><img src="{{asset('public/frontend/assets/img/mobile1.png')}}" alt="card"></div>
            <div class="slide1"><img src="{{asset('public/frontend/assets/img/mobile2.png')}}" alt="card"></div>
            <div class="slide1"><img src="{{asset('public/frontend/assets/img/mobile3.png')}}" alt="card"></div>
            <div class="slide1"><img src="{{asset('public/frontend/assets/img/mobile4.png')}}" alt="card"></div>
            <div class="slide1"><img src="{{asset('public/frontend/assets/img/mobile5.png')}}" alt="card"></div>
        </section>
    </div>

    <!-- ====== Brands Section  ======= -->
    <div class="brands-heading container">
        <hr></hr>
        <h2>Top Vehicle Brands</h2>
        <h3>NEW & USED CAR SPARES FOR ANY MAKE AND MODEL</h3>
        <div class="brands-logos container">
            @if(!empty($make))
                @foreach($make as $brand)
                    <a href="{{ url('car/spareparts/'. $brand->id) }}">
                        <img
                            src="{{Storage::disk('public')->exists('sm/'.$brand->image) ? Storage::disk('public')->url('sm/'.$brand->image) : Storage::disk('public')->url('default.png')}}"
                            alt="">
                    </a>
                @endforeach
            @endif

        </div>
    </div>

    <!-- ====== End Brands Section  ======= -->
    <!-- ====== Mobile Slider Brands Section  ======= -->
    <div class="brands-logo-mobile container">
        <h3 class="text-center font-weight-bold">Top Vehicle Brands</h3>
        <section class="customer-logos slider">
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand2.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand3.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand4.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand5.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand6.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand7.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand8.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand9.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand10.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
            <div class="slide"><img src="{{asset('public/frontend/assets/img/brand11.png')}}" alt="logo"><img
                    src="{{asset('public/frontend/assets/img/brand1.png')}}" alt="logo"></div>
        </section>

    </div>
    <!-- ====== End Mobile Slider Brands Section  ======= -->
    <!-- ====== Find your part Section  ======= -->
    <div class="find-part">
        <div class="find-part-content container">
            <h2>Find your spare part</h2>
            <input type="text" value="Enter your REG">
            <input type="submit" value="Submit">
        </div>
    </div>

    <!-- ====== End Find your part Section  ======= -->

    <!-- ====== Contact Us Section  ======= -->
    <div class="contact-us">
        <div class="separator  container ">Contact Us</div>
        <form class="container home-form">
            <input type="fname" id="fname" name="name" placeholder="Your Name" required>
            <input type="email" id="email" name="email" placeholder="Enter your Email" required>
            <input type="text" name="parts" id="parts" placeholder="Part you need (optional)">
            <input type="text" name="reg" id="reg" placeholder="Your REG Plate (optional)">
            <textarea id="subject" name="subject" placeholder="Write something.." style="height:200px"
                      required></textarea>
            <div class="contact-button">
                <input type="submit" value="Submit" name="form-button">
            </div>
        </form>
    </div>
    <!-- ====== End Contact Us Section  ======= -->

    <!-- ====== Become Supplier   ======= -->

    <div class="supplier-section">
        <div class="row supplier-row">
            <div class="col-sm-12 col-md-6 supplier-container">
                <div class="supplier-content">
                    <h2>Become a supplier!</h2>
                    <h3>Spare Car Part wants you on their books</h3>
                    <a href="#">Learn more</a>
                </div>
            </div>
            <div class="col-sm-12 col-md-6">
                <div class="supplier-image">
                    <img src="{{asset('public/frontend/assets/img/Group 686.png')}}" alt="">
                </div>
            </div>
        </div>
    </div>
    <!-- ====== End Become Supplier   ======= -->
@endsection
